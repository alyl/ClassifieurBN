...
with open('ratings_job_film.csv', 'rb') as data:
    ...
    for item in data_reader:
        ...
        if item[50]:
            user_note_StarWars.append(item)
        else:
            user_note_NonStarWars.append(item)
data.close()

with open ('ratings_job_film_StarWars.csv', 'wb') as new_file:
    new_writer = csv.writer( new_file, delimiter =',')
    for item in user_note_StarWars:
        new_writer.writerow(item)

new_file.close()

with open ('ratings_job_film_NonStarWars.csv', 'wb') as new2_file:
    new2_writer = csv.writer( new2_file, delimiter =',')
    head = ['job'] + ['film ' + str(i) for i in range(1,FilmNb+1)]
    new2_writer.writerow(head)
    for item in user_note_NonStarWars:
        new2_writer.writerow(item)

new2_file.close()
print 'ratings_job_film_StarWars.csv and rating_job_film_NonStarWars.csv created, time = ' + str(int((time.time() - start)*1000)) + 'ms'
